...
def calcular_estatisticas(ativos_por_esp):
    ...
    for item in todas_frotas:
        motivo_principal = item['motivo'].split(' + ')[0].strip()
        contagem[motivo_principal] = contagem.get(motivo_principal, 0) + 1
...
def get_programacao_agrupada(conn, data_inicio_str, data_fim_str, sincronizar_preventiva=False):
    ...
    for item in agrupado.values():
        ...
        item['dados_modal'] = "||".join([f"{s['id']}::{s['tipo']}::{s['resultado']}::{str(s['observacao']).replace(':', '').replace('|', '').replace(chr(39), '').replace(chr(34), '')}::{s['status']}" for s in item['servicos']])
...
def index():
    ...
    conn.close()
    return render_template('index.html', frotas=frotas_bloqueadas, dias_semana=dias_semana, programacao=programacao_db, hoje=hoje_data.strftime('%Y-%m-%d'), offset=offset, especialidades_na_semana=especialidades_na_semana, data_inicio_db=data_inicio_str, data_fim_db=data_fim_str, stats=stats_prog)
...
def admin():
    ...
    return render_template('admin.html', ativos=ativos, historico_agrupado=historico_agrupado, total=total_bloqueadas, contagem=contagem_motivos, frotas_cadastradas=frotas_cadastradas, especialidades_unicas=especialidades_unicas, agrupamentos_unicos=agrupamentos_unicos, setores_unicos=setores_unicos, perfis=perfis)
...
def relatorio():
    ...
    return render_template('relatorio.html', frotas=frotas_planas, data_hoje=datetime.now().strftime("%d/%m/%Y"), total=total_bloqueadas, contagem=contagem_motivos)
...
def programacao():
    ...
    return render_template('programacao.html', offset=offset, mes_offset=mes_offset, modo_visualizacao=modo_visualizacao, titulo_mes=titulo_mes, calendario_mensal=calendario_mensal, data_inicio_db=data_inicio_str, data_fim_db=data_fim_str, dias_semana=dias_semana, frotas=frotas, programacao=programacao_db, stats=stats_prog, especialidades_na_semana=especialidades_na_semana, setores_na_semana=setores_na_semana, matriz_agrupada=matriz_agrupada, matriz_setor=matriz_setor, setores_cadastrados=setores_cadastrados, hoje=hoje_data.strftime('%Y-%m-%d'), frotas_na_semana=frotas_na_semana)
...
def atualizar_servico_lote():
    ...
    for at in atualizacoes:
        ...
        conn.execute('UPDATE programacao_semanal SET status = ?, data_execucao = ?, local_execucao = ?, resultado_analise = ?, observacao = ? WHERE id = ?',
...
def relatorio_programacao():
    ...
    especialidades = get_especialidades_da_semana(prog_filtrada)
    dias_semana = get_dias_semana(datetime.strptime(data_inicio, '%Y-%m-%d'))

    agrupado_pdf = {}
    for p in prog_filtrada:
        macro = p['especialidade'] if p['especialidade'] else 'GERAL'
        micro = p['especialidade_detalhe'] if p['especialidade_detalhe'] else 'OUTROS'
        if macro not in agrupado_pdf: agrupado_pdf[macro] = {}
        if micro not in agrupado_pdf[macro]: agrupado_pdf[macro][micro] = []
        agrupado_pdf[macro][micro].append(p)

    agrupado_pdf = {k: agrupado_pdf[k] for k in sorted(agrupado_pdf)}
    for k in agrupado_pdf: agrupado_pdf[k] = {m: agrupado_pdf[k][m] for m in sorted(agrupado_pdf[k])}
    ...
    return render_template('relatorio_programacao.html', programacao=prog_filtrada, dias_semana=dias_semana, especialidades=especialidades, layout=layout, titulo=f"Fila de Programação: {titulo_tipos}", periodo=f"{data_inicio_br} a {data_fim_br}", data_geracao=data_geracao, agrupado_pdf=agrupado_pdf)
...
def dash_oleo():
    ...
    for p in programacao_db:
        for s in p.get('servicos', []):
            ...
            if 'TROCA DE OLEO' not in tipo:
                continue
            dados_oleo.append({
                'id': s.get('id'),
                'id_frota': p.get('id_frota'),
                'descricao': p.get('descricao', 'SEM DESCRIÇÃO'),
                'especialidade': p.get('especialidade', 'GERAL'),
                'tipo_servico': tipo,
                'status': s.get('status', p.get('status', 'PENDENTE')),
                'data_planejada': p.get('data_planejada'),
                'local_execucao': p.get('local_execucao', 'BASE')
            })
    resumo = get_resumo_dashboard_por_tipo(conn, data_inicio, data_fim, 'TROCA DE OLEO')
    conn.close()
    data_inicio_br = datetime.strptime(data_inicio, '%Y-%m-%d').strftime('%d/%m/%Y')
    data_fim_br = datetime.strptime(data_fim, '%Y-%m-%d').strftime('%d/%m/%Y')
    periodo = f"{data_inicio_br} à {data_fim_br}"
    data_geracao = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
...
def dash_analise():
    ...
    rows = conn.execute('''
        SELECT p.id, p.id_frota,
               COALESCE(NULLIF(TRIM(f.descricao), ''), 'SEM DESCRIÇÃO') as descricao,
               COALESCE(NULLIF(TRIM(f.agrupamento), ''), 'GERAL') as especialidade,
               p.tipo_servico, p.status, p.data_planejada,
               IFNULL(p.resultado_analise, '') as resultado_analise,
               COALESCE(NULLIF(TRIM(p.local_execucao), ''), 'BASE') as local_execucao
        FROM programacao_semanal p LEFT JOIN frotas f ON p.id_frota = f.id_frota
        WHERE p.data_planejada BETWEEN ? AND ? AND p.tipo_servico LIKE '%ANALISE DE OLEO%'
    ''', (data_inicio, data_fim)).fetchall()
    resumo = get_resumo_dashboard_por_tipo(conn, data_inicio, data_fim, 'ANALISE DE OLEO')
    conn.close()
    dados_analise = [dict(r) for r in rows]
    data_inicio_br = datetime.strptime(data_inicio, '%Y-%m-%d').strftime('%d/%m/%Y')
    data_fim_br = datetime.strptime(data_fim, '%Y-%m-%d').strftime('%d/%m/%Y')
    periodo = f"{data_inicio_br} à {data_fim_br}"
    data_geracao = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
...
def dash_preventiva():
    ...
    rows = conn.execute('''
        ...
        FROM programacao_semanal p
        LEFT JOIN frotas f ON p.id_frota = f.id_frota
        WHERE p.data_planejada BETWEEN ? AND ? AND p.tipo_servico LIKE '%PREVENTIVA%'
    ''', (data_inicio, data_fim)).fetchall()

    resumo = get_resumo_dashboard_por_tipo(conn, data_inicio, data_fim, 'PREVENTIVA')
    dados_prev = [dict(r) for r in rows]
    ...
    for i in range(4, -1, -1):
        ...
        rows_semana = conn.execute('''
            ...
            WHERE data_planejada BETWEEN ? AND ? AND tipo_servico LIKE '%PREVENTIVA%' AND status != 'QUEBRADA'
        ''', (seg_hist.strftime('%Y-%m-%d'), dom_hist.strftime('%Y-%m-%d'))).fetchall()

        total = len(rows_semana)
        realizadas = sum(1 for r in rows_semana if r['status'] == 'REALIZADA')
        perc = int((realizadas / total) * 100) if total > 0 else 0

        mes_nome = nome_meses[seg_hist.month - 1]
        label = f"{mes_nome} {seg_hist.strftime('%d/%m')} - {dom_hist.strftime('%d/%m')}"
    ...
    data_inicio_br = segunda.strftime('%d/%m/%Y')
    data_fim_br = domingo.strftime('%d/%m/%Y')
    periodo = f"{data_inicio_br} à {data_fim_br}"
    data_geracao = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
...
def dash_pitstop():
    ...
    rows = conn.execute('''
        ...
        FROM programacao_semanal p
        LEFT JOIN frotas f ON p.id_frota = f.id_frota
        WHERE p.data_planejada BETWEEN ? AND ? AND p.tipo_servico LIKE '%PIT STOP%'
    ''', (data_inicio, data_fim)).fetchall()

    dados_pit = [dict(r) for r in rows]
    _total_pit = len(dados_pit)
    _real_pit  = sum(1 for r in dados_pit if r['status'] == 'REALIZADA')
    _bloq_pit  = sum(1 for r in dados_pit if r['status'] == 'BLOQUEADA')
    _qbr_pit   = sum(1 for r in dados_pit if r['status'] == 'QUEBRADA')
    _pend_pit  = _total_pit - _real_pit - _bloq_pit - _qbr_pit
    _valido    = _total_pit - _qbr_pit
    resumo = {
        'total': _total_pit,
        'realizadas': _real_pit,
        'bloqueadas': _bloq_pit,
        'quebradas':  _qbr_pit,
        'pendentes':  _pend_pit,
        'percentual': int((_real_pit / _valido) * 100) if _valido > 0 else 0,
        'especialidades': {}
    }
    ...
    for i in range(4, -1, -1):
        ...
        rows_semana = conn.execute('''
            SELECT status FROM programacao_semanal
            WHERE data_planejada BETWEEN ? AND ?
              AND tipo_servico LIKE '%PIT STOP%'
              AND status != 'QUEBRADA'
        ''', (seg_hist.strftime('%Y-%m-%d'), dom_hist.strftime('%Y-%m-%d'))).fetchall()
        total = len(rows_semana)
        realizadas = sum(1 for r in rows_semana if r['status'] == 'REALIZADA')
        ...
        mes_nome = nome_meses[seg_hist.month - 1]
        label = f"{mes_nome} {seg_hist.strftime('%d/%m')} - {dom_hist.strftime('%d/%m')}"
        historico.append({'semana': label, 'perc': perc, 'realizadas': realizadas})
